...
function saturation() {
    let sat = 50
    let colorSat
    let colorSaturate = sat + colorSat
    ...
    for (let x = 0; x < width; x++) {
        for (let y = 0; y < height; y++) { 
            ...
            if(r > g && r > b) {
                colorSat = r
                let colorSaturate = sat + colorSat
                setPixel(imageData, x, y, colorSaturate, g, b, alpha)
            } else if(g > r && g > b) {
                colorSat = g 
                let colorSaturate = sat + colorSat
                setPixel(imageData, x, y, r,colorSaturate, b, alpha)
            } else if(b > r && b > g) {
                colorSat = b
                let colorSaturate = sat + colorSat
                setPixel(imageData, x, y, r, g, colorSaturate, alpha)
            } 
        }
    }
    ctx.putImageData(imageData, 0, 0)
